1.	Instalando MyAdmin (Para novos Projetos)
...
•	config/asset_packages.yml – Lista dos javascript e stylesheets (css) utilizado no MyAdmin, pode ser reaproveitado para ser utilizado em sua aplicação também. (Veja mais no Anexo A)
...
2.	Instalando MyAdmin ao Copiar Projeto (Github)

Para instalar uma aplicação que já utiliza o MyAdmin basta executar o comando abaixo, além dos comando básico para instalar uma aplicação Rails:
...
4.	Adicionar novo modelo no admin
...
E será adicionado também o controller  ‘controllers/my_admin/<modulo>_controller.rb’ que será responsável pelas ações do MyAdmin. Portanto se precisar alterar alguma ação padrão (index, edit, update, new, create, destroy), é esse arquivo que será alterado. (Veja mais em Alterar Ações Existente)

 
5.	Configurando Modelo
...
Tipo: Array
Ex: admin.list_display = [:name, :email, :phone]
...
o	fields – lista dos campos que o fieldset contem.
Tipo: Array

o	name – Nome do Fieldset, caso não seja definido, o nome não será exibido.
Tipo: String
...
Tipo: Array
Ex: admin.permissions = [:list, :update, :export]
...
Ex: admin.fields = {:name => {:type => :text}}

o	order:
Define se o campo poderá ser ordenado ou não na grid de visualização.
Tipo: Boolean
Ex: admin.fields = { :image => {:order => false}}

o	collection:
Definição das opções de um campo do tipo Lista (combo_box, ou list box)
Tipo: Procedure
Ex: admin.fields = { :gender => {:type => :list_string, :collection => Proc.new { ["Masculino", "Feminino"] }} }

o	start_blank:
Define se o campo do tipo Lista terá um valor em branco ou não.
Tipo: Boolean
Ex: admin.fields = { :gender => {:type => :list_string , :start_blank => false, :collection => Proc.new { ["Masculino", "Feminino"] }} }

o	class:
Nome da classe do Objeto
Tipo: String
Ex: admin.fields = { :gender => {:class => “large”} }

o	can_add:
Define se o usuário pode adicionar novos itens em um campo do tipo has_many_edit.
Tipo: Boolean
Ex: admin.fields = { :languages => { :type => :has_many_edit, :can_add => false, :can_destroy => false } }
...
•	boolean
•	clear_text 
(Campo do tipo text sem utilizar o plugin ckeditor)
•	color
•	date
•	datetime
•	decimal
...
•	read_only
•	text 
(Campo do tipo text usando o plugin ckeditor)
•	time
...
o	model: Modelo que está editando ou criado.
o	field: campo que está sendo renderizado
o	form: formulário que será enviado.
o	object: Objeto que está sendo criado ou editado.

•	/app/views/my_admin/fields/type/unidade.html.erb
Quer irá exibir no grid de visualização.
As variáveis disponíveis são:

o	model: Modelo que está visualizando.
o	field: campo que está sendo renderizado
o	object: Objeto que está visualizando.

•	/app/views/my_admin/fields/filter/unidade.hmtl.erb
Quer irá exibir no formulário de filtro.
As variáveis disponíveis são:
