...
function loadMyAdminEakroko() {
	...
	if($('.chosen-select').length > 0)
	{
		$('.chosen-select').each(function(){
			...
			opt = {no_results_text: "Nenhum resultado encontrado com ", select_some_options: "sd"};
...
$(document).ready(loadMyAdminEakroko);
document.addEventListener("page:change", loadMyAdminEakroko);
...
function apply_data_table(elem){
		// dataTables
		if($(elem).length > 0){
			$(elem).each(function(){
				var opt = {
					...
					"oLanguage":{
						...
						"sInfo": "Exibindo <span>_START_</span> a <span>_END_</span> de <span>_TOTAL_</span>",
				...
				if($(this).hasClass("dataTable-nosortall")){
					opt.bSort = false;
				}
