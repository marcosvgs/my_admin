module MyAdminModelHelper
  ...
  def collection_model_link(application, model, action, options={})
    send("#{action}_#{admin_prefix}_#{model.my_admin.url}_path", get_options_form_strong_params(options) )
  end
  ...
  def model_link(application, model, options={})
    send("#{admin_prefix}_#{model.my_admin.url}_path", get_options_form_strong_params(options))
  end

  def new_model_link(application, model, options={})
    send("new_#{admin_prefix}_#{model.my_admin.url_single}_path", get_options_form_strong_params(options))
  end

  def edit_model_link(application, model, item)
    send("edit_#{admin_prefix}_#{model.my_admin.url_single}_path", item)
  end

  def show_model_link(application, model, item)
    send("#{admin_prefix}_#{model.my_admin.url_single}_path", item)
  end
